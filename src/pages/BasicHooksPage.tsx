...
// ⚠️ 快照为便于对照学习的精简骨架（已省略样式/包装/长文案）；修改下方演示组件的核心逻辑（state/事件/API）时请同步更新。
// -----------------------------------------------------
const USESTATE_DEMO_SOURCE = `function UseStateDemo() {
  // useState：声明本地状态，返回 [值, 更新函数]
  const [count, setCount] = useState(0)
  const [user, setUser] = useState({ name: '', age: 0 })

  // 对象状态更新：先展开旧对象再覆盖（不可变更新）
  const updateAge = () => {
  ...
  return (
    <div>
      ...
      <input
        ...
        placeholder="年龄"
        value={user.age || ''}
        onChange={(e) => setUser({ ...user, age: Number(e.target.value) || 0 })}
      />
      <button onClick={updateAge}>年龄+1</button>
      <p>{user.name ? user.name + ', ' + user.age + '岁' : '(请填写用户信息)'}</p>
    </div>
  )
}`
...
const USEEFFECT_DEMO_SOURCE = `function UseEffectDemo() {
  ...
  // ① 依赖 [count]：count 变化 → 记录「挂载/更新」与「清理」日志
  useEffect(() => {
    mountedRef.current += 1
    setMountLog((prev) => ['✅ 组件挂载/更新 (第' + mountedRef.current + '次)', ...prev].slice(0, 5))
    return () => setMountLog((prev) => ['🧹 清理函数执行', ...prev].slice(0, 5))
  }, [count])
  // ② 空依赖 []：只在挂载时订阅 resize，卸载时移除监听
  useEffect(() => {
    const handleResize = () => setWidth(window.innerWidth)
    window.addEventListener('resize', handleResize)
    return () => window.removeEventListener('resize', handleResize)
  }, [])
  // ③ 依赖 [keyword]：停止输入 500ms 后执行搜索（防抖）
  useEffect(() => {
    if (!keyword) return
  ...
  return (
    <div>
      ...
      />
      <p>执行日志（最近5条）：</p>
      {mountLog.length === 0 ? <p>暂无日志...</p> : mountLog.map((log, i) => <p key={i}>{log}</p>)}
    </div>
  )
}`
...
const USECALLBACK_DEMO_SOURCE = `function UseCallbackDemo() {
  ...
  // 对照组：普通函数每次渲染都是新引用 vs 实验组：useCallback 引用稳定
  const normalHandleClick = () => console.log('普通函数被调用')
  const memoedHandleClick = useCallback(() => console.log('useCallback 函数被调用'), [])
  // 用 ref 记录上一次渲染的函数引用，判断本次是否变化并累计
  ...
  return (
    <div>
      ...
      <button onClick={() => setCount((c) => c + 1)}>修改 state（触发父组件重渲染）</button>
      <p>父组件计数：{count}</p>
      <p>普通函数：{isNormalSame ? '✅ 引用相同' : '❌ 引用变化'}（已变化 {renderCount.current.normal} 次）</p>
      <p>useCallback：{isMemoedSame ? '✅ 引用相同' : '❌ 引用变化'}（已变化 {renderCount.current.memoed} 次）</p>
      <input
        placeholder="随便输入也会触发重渲染"
        value={text}
        onChange={(e) => setText(e.target.value)}
      />
      {/* 配合 React.memo：父重渲染时 props 引用不变，子组件跳过渲染 */}
      <MemoChild callback={stableCallback} label="稳定函数" data={childState} />
      <button onClick={() => setChildState({ a: Math.random() })}>改变传给子组件的 data</button>
    </div>
  )
}
...
const MemoChild = memo(function MemoChild({ callback, label, data }: MemoChildProps) {
  ...
  return (
    <div>
      <p>🧒 memo子组件 [{label}] 渲染次数：{renderCount.current}（data.a = {data.a.toFixed(2)}）</p>
...
const USEMEMO_DEMO_SOURCE = `function UseMemoDemo() {
  ...
  // ✅ useMemo：依赖 [list, filter] 不变时跳过昂贵的重算
  ...
  // 派生值同样可以缓存（依赖 filteredWithMemo）
  const doubled = useMemo(
    () => filteredWithMemo.slice(0, 5).map((n) => n * 2),
    [filteredWithMemo]
  )
  ...
  return (
    <div>
      ...
      <button onClick={() => setForceRender((n) => n + 1)}>🔄 不相关重渲染 {forceRender}</button>

      <p>❌ 不用 useMemo：结果 {filteredWithoutMemo.length} 条，耗时 {withoutMemoTime.toFixed(2)} ms</p>
      <p>✅ 使用 useMemo：结果 {filteredWithMemo.length} 条，耗时 {withMemoTime.toFixed(2)} ms</p>
...
const USEREF_DEMO_SOURCE = `function UseRefDemo() {
  ...
  return (
    <div>
      ...
      <button onClick={focusInput}>🎯 聚焦并选中</button>
      <p>组件总渲染次数：{renderCountRef.current}（ref 自增不触发渲染）</p>
